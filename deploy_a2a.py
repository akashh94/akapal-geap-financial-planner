...
# Force environment to match our intent before ANY other imports
REGION = "us-east1"
...
if REGION != "us-east1":
    print(f"ERROR: Deployment location is {REGION}, but us-east1 was expected.")
    print("Full environment check:")
    for k, v in os.environ.items():
        if "REGION" in k or "LOCATION" in k or "PROJECT" in k:
            print(f"  {k}={v}")
    # sys.exit(1) # Commented out so it doesn't break their flow if they want to override, 
...
print(f"SDK Client Location: {client._api_client.location}")
print(f"Env GOOGLE_CLOUD_LOCATION: {os.environ.get('GOOGLE_CLOUD_LOCATION')}")
...
print(f"Searching for engines with display_name={DISPLAY_NAME!r} in {REGION}...")
...
for engine in all_engines:
    ...
    if f"/locations/{REGION}/" in name:
        ...
        print(f"  (Skipping match from different region: {name})")
...
print("\nVerifying deployment in us-east1...")
verified = [
    e.api_resource.name 
    for e in client.agent_engines.list(
        config={"filter": f'display_name="{DISPLAY_NAME}"'}
    )
    if f"/locations/{REGION}/" in e.api_resource.name
]
if verified:
    print(f"SUCCESS: Found {len(verified)} engine(s) in {REGION}:")
    for v in verified:
        print(f"  - {v}")
else:
    print(f"WARNING: Could not find engine with display_name={DISPLAY_NAME!r} in {REGION} after deployment.")
